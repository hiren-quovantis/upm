        ...
        success: function (response) {
          ...
          if(category != ''){
            ...
            for(i=0; i< categoryValue.length; i++){
              $('input[type="checkbox"][value="' + (categoryValue[i]) + '"]').prop("checked", true);
            }
  ...
  .autocomplete( "instance" )._renderItem = function( ul, item ) {
    if(item.ref != -1 ){
      return $( "<li>" )
        ...
        .append( "<div class='ui-menu-item-wrapper'>" + item.doc.Brief + " (" + item.ref + ")</div>" )
        .appendTo( ul );
    }else{
      return $( "<li>" )
...
//       if(panelBodyContainer[i].children.length  > 5){
//           panelBodyContainer[i].nextElementSibling.style.display="block"
//           $(panelBodyContainer[i]).children('.type-name:gt(4)').hide()
//       }
//     }
// }
